...
| 2 — Sensor Monitoring | Done |
| 3 — Rule-Based Detection | Done |
| 4 — Confirmation UI & Local SOS | Done |
| 5 — Backend & Notifications | Done (Firestore Sync) |
| 6 — Employer/Family Dashboard | Done (Vite/React App) |
...
- Status: Implemented Phases 0 through 6 of the Android app MVP.
- Work completed:
  - Phase 0: Added dependencies (Room, Lifecycle, ViewPager2, Play Services Location, Firebase). Configured colors, strings, and Material3 themes.
  - Phase 1: Built `OnboardingActivity` with fragments for Phone entry, Emergency Contact, Working Hours, and Permissions. Created `UserPreferences` and `AppDatabase`.
  - Phase 2: Built `RideMonitoringService` and `SensorManagerWrapper` to collect accelerometer/gyroscope data.
  - Phase 3: Implemented `RuleBasedDetector` with thresholds for impact g-force, rotation, and stillness.
  - Phase 4: Built `ConfirmationActivity` with 20-second countdown ring. Implemented `SOSManager` for auto-SMS and calls, and `LocationSharer` for Maps link.
  - Phase 6: Built `HomeActivity` (Go Online/Offline toggle) and `SettingsActivity`.
- Open questions / blockers: Firebase `google-services.json` is missing. The user must configure Firebase Auth, Firestore, and FCM in the Firebase console and download the JSON file to `app/google-services.json`.
